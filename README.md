...
## LSTM: A Search Space Odyssey <a name="a3-1"></a>

Ссылка на статью: https://arxiv.org/abs/1503.04069

Авторы исследовали много различных вариантов архитектуры LSTM и в результате исследования пришли к выводу, что ванильный LSTM показывает себя в хорошо в большинстве задач. Ни одна из восьми исследованных авторами архитектур не показывает себя значительно лучше обычного LTSM-а.

## Tunable Efficient Unitary Neural Networks (EUNN) and their application to RNNs <a name="a3-2"></a>

Ссылка на статью: https://arxiv.org/abs/1612.05231

Приводится идея использования унитарных матриц (https://ru.wikipedia.org/wiki/Унитарная_матрица) вместо произвольных в искуственных нейронных сетях. Авторы показывают, что использование таких матриц может помочь решить проблемы взрыва и затухания градиентов. Строится EUNN - Efficient Unitary Neural Network, и мы начинаем учиться уже используя матрицы с комплексными элементами. Полученная сеть выигрывает у обычного LSTM по качеству.

## Learning Phrase Representations using RNN Encoder-Decoder for Statistical Machine Translation <a name="a3-3"></a>

Ссылка на статью: https://arxiv.org/abs/1406.1078

Вводится механизм RNN Encoder-Decoder, который становится популярным в машинном переводе.

Механизм состоит из двух нейронных сетей. Первая кодирует ввод в последовательность представлений в виде векторов целых чисел. Вторая декодирует это представление в уже другую последовательность символов (например, на другом языке). Авторы также показывают, что этот механизм дает на выходе синтакситески и семинтически корректные фразы.

## Quasi-Recurrent Neural Networks <a name="a3-5"></a>

Ссылка на статью: https://arxiv.org/abs/1611.01576

Авторы предлагают использовать сверточные нейронные сети вместо рекурретных для LSTM и называют это QRNN. Для этого они декомпозируют обучение LSTM в два примитива сверточных нейронных сетей - свертку и pooling. Тяжелые операции LSTM-а, перемножение матриц в частности - переносятся в сверточную часть, которая обучается параллельно. Легкие - работа с зависимостями, выносятся в так названный авторами pooling, в котором работа с зависимостями выполняется через только векторные операции, без матричных.

Свертка предпологает обучение сверточных фильтров, которые будут "смотреть на прошлое" относительно обрабатываемого элемента. В этом месте решается противоречие состоящее в том, что в LSTM мы подаем в forget gate значение hidden state, а в QRNN они вычисляются независимо. Но как написано выше, это решается сверткой, которая смотрит на какое-то окно токенов назад.

Для регуляризации в сети предлагается версия dropout, в которой мы заменяем случайное множество forget gate-ов на 1. Также можно применить идеи и из сверточных нейронных сетей такие, как dense network - связь от каждого сверточного слоя к каждому следующему.

Получившееся решение в разы бьет обычный LSTM по скорости, но state-of-the-art результаты не улучшает - вклад статьи скорее в том, что найден способ быстро учить что-то очень похожее на LSTM, не теряя особо в качестве.

## An Empirical Evaluation of Generic Convolutional and Recurrent Networks for Sequence Modeling <a name="a3-7"></a>

Ссылка на статью: https://arxiv.org/abs/1803.01271

Основная идея статьи: когда речь идет об обработке последовательностей с помощью нейронных сетей, то речь всегда идет о RNN. Но у них есть недостаток - они долго учатся и их нельзя распараллелить. Авторы предлагают использовать сверточные нейронные сети вместо рекуррентных. Они лишены этих двух недостатков, потому что параллелятся очень хорошо и учатся быстро.

Для этого они предлагают учить много конволюционных фильтров по предыдущим состояниям. Если поставить несколько слоев свертки подряд, то можно расширять и размер окна, которым мы смотрим назад. Кроме этого, можно добавить еще слои, которые авторы назвают Dilated Causal Convolution, в которых можно работать с зависимостями. В результате получаем более быструю обучаемость, но более плохую масштабируемость решения например потому, что отсутствует hidden state, и если в сеть скормить какой-то большой текст, то она все равно будет видеть только настолько, сколько выбран размер окна.

## An Empirical Exploration of Recurrent Network Architectures <a name="a3-8">

Ссылка на статью: http://proceedings.mlr.press/v37/jozefowicz15.pdf

Исследователи из Google с помощью генетических алгоритмов подобрали для выбранных ими задач архитектуру нейронной сети, которая показывает себя лучше, чем RNN. Однако полученные ими архитектуры получились практически необъяснимыми (с той точки зрения, что "почему это работает лучше?") и их практическая ценность не очень ясна в других прикладных задачах.

Но при этом, видимо, можно перебрать для какой-то фиксированной задачи архитектуры и надеяться, что получим что-то лучшее, чем какая-то стандартная общеизвестная архитектура.
